...
const AdminAddShelter = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.104:5000/api/shelters', {
        method: 'POST',
      ...
      if (!response.ok) throw new Error('Failed to add shelter');
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Accommodation (e.g. 50 beds available)" value={accommodation} onChangeText={setAccommodation} />
      <TextInput style={styles.input} placeholder="Timings" value={timings} onChangeText={setTimings} />
      <Button mode="contained" style={styles.button} onPress={handleSubmit}>Add Shelter</Button>
    </ScrollView>
  );
};
...
export default AdminAddShelter; 